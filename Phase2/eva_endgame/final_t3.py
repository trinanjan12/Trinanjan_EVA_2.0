    ...
    def __init__(self, action_dim, max_action):
        ...
        def conv_block_first(in_dim, out_dim):
            model = nn.Sequential(
                ...
                nn.ReLU(inplace=True),
                nn.Dropout(.2),
                nn.Conv2d(32, 16, kernel_size=1, stride=2, padding=1),
                nn.ReLU(inplace=True),
                nn.AvgPool2d(9),
                nn.Flatten(),
        ...
        self.conv_img = conv_block_first(1, 32)
        concat_input = 32 + 2 
        self.layer_1 = nn.Linear(concat_input,100)
        self.layer_2 = nn.Linear(100, action_dim)

    def forward(self, input_1 , input_2):
        x1 = self.conv_img(input_1)
        x =  torch.cat([x1,input_2], 1)
        x = F.relu(self.layer_1(x))
        return max_action * torch.tanh(self.layer_2(x) * 100) # need to check the temperature value
...
class TD3(object):
    ...
    def train(self, replay_buffer, iterations, batch_size=100, discount=0.99, tau=0.005, policy_noise=0.2, noise_clip=0.5, policy_freq=2):
        ...
        for it in range(iterations):
            ...
            state_2 = torch.Tensor(batch_states2).to(self.device)
            next_state_1 = torch.Tensor(batch_next_states1).float().to(self.device)
            next_state_2 = torch.Tensor(batch_next_states2).to(self.device)
            ...
            target_Q1, target_Q2 = self.critic_target(next_state_1,next_state_2, next_action)
            ...
            if it % policy_freq == 0:
                actor_loss = -self.critic.Q1(state_1, state_2,self.actor(state_1,state_2)).mean()
    ...
    def update(self, reward, new_signal):
        ...
        if self.done:
            ...
            if self.total_timesteps % 1e4 == 0:
                self.policy.save("%s" % (file_name), directory="./car_models_new")
